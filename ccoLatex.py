...
from sympy                      import asin, acos, atan, sin, cos, tan, oo, sinc
...
class PyLatex():
    ...
    def Adjust_Latex(self,s): #adjust sympy LaTex for matplot latex
        tx='$'+latex(s)+' $'
        #adjustment on sympy Latex for compatibility with pyplot
        return tx

    def Text(self,s,indent="\\noindent "):  #Append text to Frame variable
        return indent+s+"\n"
    ...
    def Split_Expression(self,s,n):
        '''
        s is the expression to be split into n nunber.
        '''
        l=len(s.args);
        seg=int(l/n)
        rng=[]
        for i in range(n-1):
            rng.append(range(i*seg,(i+1)*seg))
        rng.append(range((i+1)*seg,l))
        print(rng)
        var=[]
    ...
    def Tabular(self,tbl,caption='',align="c"):
        row=len(tbl);
        col=len(tbl[0]);
...
\\maketitle\n\n'
    ...
    def Append_Equation(self,lhs,rhs,e=False,p="",c=""):
        ...
        #c is condition
        #Append equation to eQ variable and return its latex format
        self.Frame.append(self.Equation(lhs,rhs,val=e,p=p,c=c))

    def Insert_Figure(self,filename,place="H",width=1,height=.7,caption='',
                      end="\\end{figure}\n\n"):
...
\\maketitle\n\n"
    ...
    def Append_Equation(self,lhs,rhs,e=False,p="",c=""):
        ...
        #c is condition
        #Append equation to eQ variable and return its latex format
        self.TxData.append(self.Equation(lhs,rhs,val=e,p=p,c=c))
